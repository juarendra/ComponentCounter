...
2. **TFLite Model** — `best_float32.tflite` loaded from assets. YOLOv8n anchor-free, input 416×416, output `[1,8,3549]`, normalized 0..1 coords, letterbox preprocessing. Detects 4 classes in **data.yaml order**: Condensator (0), Diode (1), Resistor (2), Transistor (3) — the app `labels` list must match this exactly. Detector threshold 0.25. Retrained 2026-06-30 (mAP50 0.993). See `ml-training/MODEL_IO.md` for the verified I/O contract; YOLOv8 training notebook at `ml-training/train_yolo.ipynb`.
